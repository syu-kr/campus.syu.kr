...
def extract_scholarship_row(row):
    """장학공지 행에서 정보 추출"""
    try:
        # 다양한 선택자 시도
        ...
        if len(tds) >= 5:
            author = tds[1].get_text(strip=True) if tds[1] else ""
            date_text = tds[2].get_text(strip=True) if tds[2] else ""
...
def crawl_scholarship_notice():
    ...
    if os.path.exists(data_path):
        try:
            with open(data_path, 'r', encoding='utf-8') as f:
                ...
                if existing_notices:
                    latest_item = existing_notices[0]
                    print(f"📌 저장된 최신글: \"{latest_item['title']}\" ({latest_item['date']}, {latest_item['author']})")
                    
                    for notice in existing_notices:
                        key = f"{notice['title']}|{notice['date']}|{notice['author']}"
                        existing_map[key] = notice['id']
        except Exception as e:
    ...
    try:
        ...
        for idx, row in enumerate(table_rows[:20]):  # 첫 20개만 검증
            row_data = extract_scholarship_row(row)
            if not row_data:
                continue
            ...
            try:
                ...
                for row in table_rows:
                    ...
                    if key in existing_map:
                        notice_id = existing_map[key]
                    else:
                        notice_id = generate_stable_id("scholarship", row_data['title'], 
