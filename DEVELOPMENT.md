...
| `ANNOUNCEMENT_AI_TIMEOUT_MS` | 선택 | announcement AI summary script | OpenAI 요청 timeout(ms), 기본값 `30000` |
| `ANNOUNCEMENT_AI_CHECKPOINT_EVERY` | 선택 | announcement AI summary script | 장시간 백필 중 메타데이터 중간 저장 주기, 기본값 `5` |
| `ANNOUNCEMENT_AI_MAX_RETRIES` | 선택 | announcement AI summary script | OpenAI SDK 재시도 횟수, 기본값 `3` |
| `ANNOUNCEMENT_AI_REFRESH_GENERATION` | 선택 | announcement AI summary script | `true`일 때만 모델/프롬프트 generation fingerprint가 다른 기존 요약을 선택 백필 |
| `ANNOUNCEMENT_AI_REFRESH_SINCE` | 선택 백필 시 필수 | announcement AI summary script | 선택 백필의 최소 공지일. `YYYY-MM-DD` 형식이며 limit과 함께 범위를 제한 |
...
| `ANNOUNCEMENT_AI_TIMEOUT_MS` | 선택 | `crawl-daily.yml` | OpenAI 요청 timeout(ms), 기본값 `30000` |
| `ANNOUNCEMENT_AI_CHECKPOINT_EVERY` | 선택 | `crawl-daily.yml` | 장시간 생성 중 중간 저장 주기 |
| `ANNOUNCEMENT_AI_MAX_RETRIES` | 선택 | `crawl-daily.yml` | OpenAI SDK 일시 오류 재시도 횟수 |
| `ANNOUNCEMENT_AI_REFRESH_GENERATION` | 수동 선택 | `crawl-daily.yml` | 선택 백필 활성화. workflow dispatch 입력으로만 사용 |
| `ANNOUNCEMENT_AI_REFRESH_SINCE` | 수동 선택 | `crawl-daily.yml` | 선택 백필 최소 공지일. workflow dispatch 입력으로만 사용 |
...
`public/data/announcement-ai-metadata.json`은 공지 원문 JSON을 수정하지 않고 AI 요약만 별도 저장합니다. `scripts/generate_announcement_ai_summaries.mjs`는 `.env.local`과 `.env`를 읽고 OpenAI Responses API로 신규·변경 공지만 처리합니다. `OPENAI_API_KEY`가 없거나 개별 호출이 실패하면 source hash가 유효한 기존 요약을 보존하며, 크롤 데이터 배포는 계속됩니다. 공지 JSON 본문이 비어 있으면 `syu.ac.kr` 상세 URL에 접속해 본문 영역을 추출한 뒤 AI 입력으로 사용하며, 상세 페이지를 읽지 못하면 제목/작성자/날짜/URL만으로 낮은 confidence 요약을 생성합니다. 기존 요약은 `id`가 아니라 query/hash를 제거한 공지 상세 URL을 우선 키로 사용하고, 공지 제목/작성자/날짜/본문/canonical URL 해시가 일치할 때만 화면에 병합됩니다. `inputHash`는 공지 내용 변경만 추적하며 모델·프롬프트 변경은 자동 재생성 조건이 아닙니다. 기존 결과의 선택 백필은 `ANNOUNCEMENT_AI_REFRESH_GENERATION=true`, `ANNOUNCEMENT_AI_REFRESH_SINCE`, 실행 limit을 함께 지정한 경우에만 수행합니다.
