...
| `GYEONGGI_BUS_LOCATION_URL` | 필수 | 경기도 버스 위치 endpoint |
...
| `ADMIN_EMAILS` | 권장 | 관리자 허용 이메일 목록. 쉼표로 구분 |
...
| `NEXT_PUBLIC_APP_URL` | 선택 | 공개 앱 URL. 현재 핵심 런타임 코드에서는 직접 사용하지 않음 |
...
## Optional Graduation Crawler Variables

현재 GitHub Actions 예약 워크플로에는 연결되어 있지 않지만, 로컬에서 `scripts/crawl_graduation_requirements.py`를 실행하려면 아래 값을 환경변수로 설정해야 합니다.

| 이름 | 설명 |
| --- | --- |
| `CRAWL_GRAD_BUSINESS_URL` | 경영학과 졸업요건 URL |
| `CRAWL_GRAD_GLOBAL_KOREAN_URL` | 글로벌한국학과 졸업요건 URL |
| `CRAWL_GRAD_COMPUTER_SCIENCE_URL` | 컴퓨터공학부 졸업요건 URL |
| `CRAWL_GRAD_NURSING_URL` | 간호학과 졸업요건 URL |
| `CRAWL_GRAD_PHARMACY_URL` | 약학과 졸업요건 URL |
| `CRAWL_GRAD_ARCHITECTURE_URL` | 건축학과 졸업요건 URL |
| `CRAWL_GRAD_AI_URL` | 인공지능융합학부 졸업요건 URL |
