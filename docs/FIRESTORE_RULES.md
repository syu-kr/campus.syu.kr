...
Firestore Rules의 소스 오브 트루스는 저장소 루트의 `firestore.rules`입니다. Firebase CLI 배포를 권장하지만, 운영자가 Firebase 콘솔에서 직접 붙여넣어 반영해도 됩니다. 단, 콘솔에 넣는 내용은 반드시 `firestore.rules`와 같아야 하며, 수동 변경 후 repo 파일도 같은 상태로 유지해야 합니다.

현재 저장소는 Firestore indexes를 코드로 관리하지 않습니다. Firebase CLI를 쓰는 경우 운영 인덱스에 영향을 주지 않도록 rules만 배포합니다.
